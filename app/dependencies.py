...
async def get_current_user(
    ...
    # Only load participant_profile — single join, only populated for participants
...
async def get_current_judge_or_admin(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in [UserRole.JUDGE, UserRole.ADMIN]:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Forbidden: Admin or Judge access required")
    return current_user
